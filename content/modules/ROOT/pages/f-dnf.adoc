...
Software in Red Hat Enterprise Linux are bundled into something called an RPM.  At a highlevel an rpm file includes both 
...
A repository (or repo) is a collection of rpm packages.   More on this topic in the Intermediate Package Management unit.
...
Simple command to list all available packages available.  This would include all configured and available repos.
...
Quickly comparison of how many packages are installed vs available.
