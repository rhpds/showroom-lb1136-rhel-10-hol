...
So for this exercise, we leverage something called a 'Bind-Mount' to effectively mirror a portion of the host's filesystem into the container's namespace.  Bind-Mounts are declared using the '-v' option.  In the example below, /usr/bin and /usr/lib64 from the host will be exposed and accessible to the containers namespace mounted at '/usr/bin' (ie: /usr/bin:/usr/bin).
