...
NOTE: nft leverages special characters (i.e. curly braces and semicolons) so we must use single quotes to protect our rules on the command-line.
...
So what does the previous rule do?  It merely counts packets that arrive at port 80 on our machine.  Let's send some packets.
