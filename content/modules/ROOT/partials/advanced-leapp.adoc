...
Because the upgrade process is time consuming lengthy process, this exercise uses the seconday terminal.  
...
NOTE: In most cases, you should disable any non-Red Hat repositories prior to starting the in-place upgrade process.  Application upgrade(s) will need to take place after the operating system upgrade has completed.  A sample command to disable a repo looks like this `dnf config-manager --set-disabled <my-custom-application-repo-name>`
...
Using the second terminal reconnect to the `leapp` node once it has rebooted (
